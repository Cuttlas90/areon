...
c1, c2 = st.columns(2)
with c1:
    QUERY_STRING = """SELECT DATE(blocks.timestamp) AS transaction_date, COUNT(hash) AS daily_deploy_contracts
                    FROM contracts
                    JOIN blocks ON contracts.block_number = blocks.number
                    GROUP BY transaction_date
                    ORDER BY transaction_date;"""
    contract_history = areon_query(QUERY_STRING)
    contract_history = pd.DataFrame(contract_history, columns=['transaction_date', "daily_deploy_contracts"])
    contract_history["daily_deploy_contracts"] = contract_history["daily_deploy_contracts"].astype(int)
    contract_history['date_column'] = pd.to_datetime(contract_history['transaction_date'], format="%Y-%m-%d").dt.date
    contract_history_slider = create_slider(contract_history, "contract_history", "contract history range:")

    st.markdown('### contracts deploy History')
    filtered_df = contract_history[contract_history['date_column'].between(contract_history_slider[0], contract_history_slider[1])]
    st.line_chart(filtered_df, x = 'date_column', y = "daily_deploy_contracts", height = plot_height)
with c2:
    QUERY_STRING = """SELECT DATE(blocks.timestamp) AS transaction_date, COUNT(hash) AS daily_deploy_tokens
                    FROM tokens
                    JOIN blocks ON tokens.block_number = blocks.number
                    GROUP BY transaction_date
                    ORDER BY transaction_date;"""
    # token_history = areon_query(QUERY_STRING)
    # token_history = pd.DataFrame(token_history, columns=['transaction_date', "daily_deploy_tokens"])
    # token_history["daily_deploy_tokens"] = token_history["daily_deploy_tokens"].astype(int)
    # token_history['date_column'] = pd.to_datetime(token_history['transaction_date'], format="%Y-%m-%d").dt.date
    # token_history_slider = create_slider(token_history, "token_history", "tokens history range:")

    # st.markdown('### tokens deploy History')
    # filtered_df = token_history[token_history['date_column'].between(token_history_slider[0], token_history_slider[1])]
    # st.line_chart(filtered_df, x = 'date_column', y = "daily_deploy_tokens", height = plot_height)

c1, c2 = st.columns(2)
with c1:

    QUERY_STRING = """SELECT DATE(block_timestamp) AS transaction_date, COUNT(hash) AS daily_transaction_count
                    FROM transactions
                    GROUP BY transaction_date
                    ORDER BY transaction_date"""
    ...
    transaction_history["daily_transaction_count"] = transaction_history["daily_transaction_count"].astype(int)
...
with c2:
    QUERY_STRING = """SELECT DATE(block_timestamp) AS transaction_date, COUNT( DISTINCT from_address) AS daily_active_address
                    FROM transactions
                    GROUP BY transaction_date
                    ORDER BY transaction_date"""
    ...
    daily_active_address_history["daily_active_address"] = daily_active_address_history["daily_active_address"].astype(int)
...
c1, c2 = st.columns(2)
with c1:
    QUERY_STRING = """SELECT DATE(block_timestamp) AS transaction_date, SUM(value) AS daily_eth_number_circulate
                    FROM transactions
                    GROUP BY transaction_date
                    ORDER BY transaction_date"""
    ...
    daily_transaction_value_history["daily_eth_number_circulate"] = daily_transaction_value_history["daily_eth_number_circulate"].astype(float)
...
with c2:
    ...
    daily_new_address_history["new_from_addresses"] = daily_new_address_history["new_from_addresses"].astype(int)
...
c1, c2 = st.columns(2)
with c1:
    QUERY_STRING = """SELECT DATE(block_timestamp) AS transaction_date, SUM(gas) AS daily_gas_paid
                    FROM transactions
                    GROUP BY transaction_date
                    ORDER BY transaction_date;"""
    ...
    daily_gas_paid_history["daily_gas_paid"] = daily_gas_paid_history["daily_gas_paid"].astype(int)
